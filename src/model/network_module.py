...
class ParametersClassifier(pl.LightningModule):
    def __init__(
        ...
        self.train_acc0 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.train_acc1 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.train_acc2 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.train_acc3 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.val_acc = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.val_acc0 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.val_acc1 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.val_acc2 = tm.Accuracy(task='multiclass', num_classes=num_classes)
        self.val_acc3 = tm.Accuracy(task='multiclass', num_classes=num_classes)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.val_output_list += {"preds": preds, "targets": y}
        return loss
    ...
    def on_validation_epoch_end(self):
        ...
        os.makedirs(path.join(CODE_DIR, "test/"), exist_ok=True)
        if self.test_overwrite_filename:
            torch.save(preds, path.join(CODE_DIR, "test/preds_test.pt"))
            torch.save(targets, path.join(CODE_DIR, "test/targets_test.pt"))
        else:
            date_string = datetime.now().strftime("%H-%M_%d-%m-%y")
            torch.save(preds, path.join(CODE_DIR, "test/preds_{}.pt".format(date_string)))
            torch.save(targets, path.join(CODE_DIR, "test/targets_{}.pt".format(date_string)))
